...
async def test_api_key(api_preset: dict) -> bool:
    ...
    # 检查参数合规
    if (
        "api_key" not in api_preset
        or "base_url" not in api_preset
        or "model_name" not in api_preset
    ):
        return False

    client = AsyncOpenAI(
        api_key=api_preset["api_key"], base_url=api_preset["base_url"], timeout=10
    )
    try:
